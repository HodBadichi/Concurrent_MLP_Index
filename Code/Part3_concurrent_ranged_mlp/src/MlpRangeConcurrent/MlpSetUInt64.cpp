...
#define SAFE_STORE(var, val) (var).store((val), std::memory_order_release)
#define SAFE_LOAD(var) (var).load(std::memory_order_acquire)

/* This is safe by being visible to all threads and not by atomicity. We don't
 * need atomic fetch and add since we only have single writer)
 */
#define SAFE_ADD(var, val) SAFE_STORE((var), SAFE_LOAD((var)) + (val))
#define INC_VERSION(var) SAFE_ADD((var), 1)
#define COND_INC_VERSION(var) SAFE_ADD((var), SAFE_LOAD(var)%2)
#define COND_LOCKED_VERSION(var) SAFE_ADD((var), 1-SAFE_LOAD(var)%2)

#define __ValidateVersion(K) for (int i = K; i < 7; i++) {\
                if (read_set[0][i] != ptr_db[0][i]->version_lock) { \
                    goto _retry;                                                       \
                }\
                if (read_set[1][i] != ptr_db[1][i]->version_lock)\
                {                                          \
                    goto _retry;\
                }                                      \
            }
#define ValidateVersion() __ValidateVersion(2)
#define IS_VAL_COVERED_BY_TOP_LEVELS(value) ((m_root[(((value) >> 48) >> 8) / 64] & (uint64_t(1) << ((((value) >> 48) >> 8) % 64))) != 0) \
            &&  ((m_treeDepth1[((value) >> 48) / 64] & (uint64_t(1) << (((value) >> 48) % 64))) != 0) \
                &&  ((m_treeDepth2[((value) >> 40) / 64] & uint64_t(1) << (((value) >> 40) % 64)) != 0)
#define NAIVE_RETURN_IF_KEY_EXISTS_IN_RANGE() \
            if (!ignore_range_mechanic && (z/8 >= dlen)) {\
                uint32_t next_byte = get_byte(key, dlen + 1);\
                if (ht[allPositions1[len]].BitMapIsExist(next_byte) == 1) {\
                    return 9;\
                }\
            }

#define RETURN_IF_KEY_EXISTS_IN_RANGE() \
            if (!ignore_range_mechanic && (z/8 >= dlen)) {\
                uint32_t next_byte = get_byte(key, dlen + 1);\
                if (ht[allPositions1[len]].BitMapIsExist(next_byte) == 1) { \
                    ValidateVersion()                    \
...
//
    ...
    void CuckooHashTableNode::ExtendToBitMap() {
        ...
        if (offset == 4) {
            ...
            memset(ptr, 0, sizeof(CuckooHashTableNode) - 1);
    ...
    uint64_t *CuckooHashTableNode::CopyToExternalBitMap() {
        ...
        int offset = (hash >> 21) & 7;
        ptr[0] = childMap;
        memcpy(ptr + 1, &(this[offset - 4]), sizeof(CuckooHashTableNode) - 1);
    ...
    void CuckooHashTableNode::MoveNode(CuckooHashTableNode *target) {
        // Copy *this to target node
        target->hash = this->hash;
        target->minvOffset = this->minvOffset;
        target->minKey = this->minKey;
        target->childMap = this->childMap;

        if (IsUsingInternalChildMap() || IsExternalPointerBitMap()) {
            memset(this, 0, sizeof(CuckooHashTableNode) - 1);
            return;
        }
        int offset = (hash >> 21) & 7;
        ...
        if (targetOffset != 4) {
            memcpy(&(target[targetOffset - 4]), &(this[offset - 4]), sizeof(CuckooHashTableNode) - 1);
        } else {
            uint64_t *ptr = CopyToExternalBitMap();
            target->childMap = reinterpret_cast<uint64_t>(ptr);
        }
        memset(this, 0, sizeof(CuckooHashTableNode) - 1);
        memset(&(this[offset - 4]), 0, sizeof(CuckooHashTableNode) - 1);
#ifndef NDEBUG
    ...
    void CuckooHashTableNode::RelocateBitMap() {
        ...
        if (offset == 4) {
            ...
            memcpy(&(this[offset - 4]), &(this[oldOffset - 4]), sizeof(CuckooHashTableNode) - 1);
        }
        memset(&(this[oldOffset - 4]), 0, sizeof(CuckooHashTableNode) - 1);
        hash &= 0xfff1fffffU;
        hash |= offset << 21;
        assert(offset == 4 || (this[offset - 4].IsOccupied() && !this[offset - 4].IsNode()));
    }

    void CuckooHashTableNode::GetChildMapStr(char *s) {
        if (IsLeaf()) {
            sprintf(s, "A leaf...");
            return;
        }
    ...
    void CuckooHashTableNode::PrintSelf() {
	    char s[72] = "\0";
	    GetChildMapStr(s);
	    std::cout << "\tHash:\t\t\t" << "0x" << std::hex << hash << std::endl;
	    std::cout << "\tHash18bit:\t\t" << "0x" << std::hex << GetHash18bit() << std::endl;
	    std::cout << "\tFull key:\t\t" << "0x" << std::hex << GetFullKey() << std::endl;
	    std::cout << "\tIndex key:\t\t" << "0x" << std::hex << GetIndexKey() << std::endl;
	    std::cout << "\tOccupy flag:\t\t" << GetOccupyFlag() << std::endl;
	    std::cout << "\tIndex len:\t\t" << GetIndexKeyLen() << std::endl;
	    std::cout << "\tFullkey len:\t\t" << GetFullKeyLen() << std::endl;
	    std::cout << "\tChild map:\t\t" << s << std::endl;
	    std::cout << "\tInternal BMap:\t\t" << IsUsingInternalChildMap() << std::endl;
	    std::cout << "\tVersion Lock:\t\t" << std::hex << (int) version_lock << std::endl;
    }
    ...
    int ALWAYS_INLINE CuckooHashTable::QueryLCP(uint64_t key,
                                                ...
                                                uint32_t *read_set_out,
                                                CuckooHashTableNode **ptr_db_out,
        ...
        uint32_t read_set[2][8] = {0};
        CuckooHashTableNode *ptr_db[2][8];
        ...
        *reinterpret_cast<uint64_t *>(expectedHash + 2) = h5;
        ...
        for (; len >= 2; len--) {
            read_set[0][len] = ht[allPositions1[len]].version_lock; // allPositions1
            ptr_db[0][len] = &ht[allPositions1[len]];
            read_set[1][len] = ht[allPositions2[len]].version_lock; // allPosition2
            ptr_db[1][len] = &ht[allPositions2[len]];

            if (read_set[0][len] % 2 || read_set[1][len] % 2) // retry if slot is in inconsistent mode
            {
                goto _retry;
            }
        }

        if (read_set_out != nullptr && ptr_db_out != nullptr) {
            memcpy(read_set_out, read_set, sizeof(read_set));
            memcpy(ptr_db_out, ptr_db, sizeof(ptr_db));
        }
...
#endif
            //TODO : discuss if validation is needed in this case.
            ValidateVersion()
            return 2;
        }
...
#endif
        int dlen = ht[allPositions1[len]].GetFullKeyLen();
    ...
    int ALWAYS_INLINE CuckooHashTable::NaiveQueryLCP(uint64_t key,
                                                     uint32_t &idxLen,
                                                     uint32_t *allPositions1,
                                                     uint32_t *allPositions2,
                                                     uint32_t *expectedHash,
        ...
        *reinterpret_cast<uint64_t *>(expectedHash + 2) = h5;
...
#endif
        {
            ...
            NAIVE_RETURN_IF_KEY_EXISTS_IN_RANGE()
            return z / 8;
        }
...
#endif
            INC_VERSION(ht[h1].version_lock);
            //ht[h1].version_lock.fetch_add(1, std::memory_order_acquire);

            const bool old_victim_state = SAFE_LOAD(ht[victimPosition].version_lock) % 2;
            //const bool old_victim_state = ht[victimPosition].version_lock % 2;

            SAFE_ADD(ht[victimPosition].version_lock, 1 - old_victim_state);
            //ht[victimPosition].version_lock.fetch_add(1 - old_victim_state, std::memory_order_acquire);

            ht[victimPosition].MoveNode(&ht[h1]);

            SAFE_ADD(ht[victimPosition].version_lock, 1 - old_victim_state);
            //ht[victimPosition].version_lock.fetch_add(1 - old_victim_state, std::memory_order_release);
...
#endif
            const bool old_owner_state = owner->version_lock % 2;

            owner->version_lock += 1 - old_owner_state;
    ...
    void MlpSet::GetChildMapStr(char *s, CuckooHashTableNode *Node) {
	    Node->GetChildMapStr(s);
    }

    void MlpSet::PrintMlpNode(CuckooHashTableNode *current_node) {
	    current_node->PrintSelf();
    }
    ...
    void MlpSet::PrintMlpSet() {
        ...
        for (uint32_t i = 0; i < nodes_num; i++) {
            ...
            if (!((current_node)->IsOccupied()) || !((current_node)->IsNode()))
                continue;
            cnt++;
	    std::cout << "Node Index:\t\t" << "0x" << std::hex << i << std::endl;
	    this->PrintMlpNode(current_node);
	    std::cout << std::endl;
        }
        std::cout << "#Root:\t\t" << this->m_root_version << std::endl;
        std::cout << "#TD1:\t\t" << this->m_treeDepth1_version << std::endl;
        std::cout << "#TD2:\t\t" << this->m_treeDepth2_version << std::endl;
    std::cout << std::endl;
        std::cout << "Total Nodes: " << cnt << std::endl;
    std::cout << std::endl << std::endl << std::endl;
    }

    uint64_t MlpSet::CountNodes() {
	    CuckooHashTableNode *head = this->m_hashTable.ht;
	    unsigned long nodes_num = this->m_hashTable.htMask;
	    uint64_t cnt;
	    for (uint32_t i = 0; i < nodes_num; i++) {
    ...
    MlpSet::MlpSet()
            : m_memoryPtr(nullptr), m_allocatedSize(-1), m_hashTable(), m_treeDepth1_version(0),
              m_treeDepth2_version(0),
              m_root_version(0)
#ifndef NDEBUG
    , m_hasCalledInit(false)
#endif
    ...
    void MlpSet::Init(uint32_t maxSetSize) {
        ...
        //
        pthread_spin_init(&(m_globalLock.m_lock), PTHREAD_PROCESS_SHARED);
    ...
    bool
    MlpSet::Insert(uint64_t value,uint32_t* parent_pos, uint32_t *inserted_leaf_pos, CuckooHashTableNode **insert_write_set) {

        uint32_t *allPositions1;
        uint32_t *allPositions2;
        uint32_t ilen = 0;
        uint32_t ilen_true_value = 0;
        uint32_t x;
        bool x_used = false;
...
#define LOCK_TOP_LEVELS() INC_VERSION(this->m_root_version);\
    INC_VERSION(this->m_treeDepth1_version);\
    INC_VERSION(this->m_treeDepth2_version);

#define UNLOCK_TOP_LEVELS() COND_INC_VERSION(this->m_root_version);\
    COND_INC_VERSION(this->m_treeDepth1_version);\
    COND_INC_VERSION(this->m_treeDepth2_version);

        {
            uint64_t h16bits = value >> 48;
            if (unlikely((m_treeDepth1[h16bits / 64] & (uint64_t(1) << (h16bits % 64))) == 0)) {
                if (insert_write_set != nullptr) {
                    LOCK_TOP_LEVELS()
                }
        ...
        //
        {
            ...
            allPositions1 = reinterpret_cast<uint32_t *>(_allPositions1);
            allPositions2 = reinterpret_cast<uint32_t *>(_allPositions2);
            uint32_t *expectedHash = reinterpret_cast<uint32_t *>(_expectedHash);
            lcpLen = m_hashTable.NaiveQueryLCP(value,
                                               ilen /*out*/,
                                               allPositions1 /*out*/,
                                               allPositions2 /*out*/,
                                               expectedHash /*out*/);
            ilen_true_value = ilen;

#define LOCK_PATH_FROM_LCP() \
            for (int j = ilen_true_value; j > 2; j--) {\
                uint32_t pos1 = allPositions1[j - 1];\
                uint32_t pos2 = allPositions2[j - 1];  \
                if(insert_write_set != nullptr)                       \
                {                    \
                        COND_LOCKED_VERSION(m_hashTable.ht[pos1].version_lock);\
                        COND_LOCKED_VERSION(m_hashTable.ht[pos2].version_lock);\
                        insert_write_set[j-1] = &m_hashTable.ht[pos1];            \
                        insert_write_set[j-1 + 8] = &m_hashTable.ht[pos2];            \
                        }                    \
                else                 \
                {                    \
                        INC_VERSION(m_hashTable.ht[pos1].version_lock);\
                        INC_VERSION(m_hashTable.ht[pos2].version_lock);\
                }\
            }
#define UNLOCK_PATH_FROM_LCP() \
            for (int j = ilen_true_value; j > 2; j--) {\
                uint32_t pos1 = allPositions1[j - 1];\
                uint32_t pos2 = allPositions2[j - 1];  \
        COND_INC_VERSION(m_hashTable.ht[pos1].version_lock);\
        COND_INC_VERSION(m_hashTable.ht[pos2].version_lock);\
            }
            ...
            if (lcpLen > 2) {
                ...
                if (lcpLen == m_hashTable.ht[pos].GetFullKeyLen()) {
                    ...
                    //
                    {
                        ...
                        x = m_hashTable.ReservePositionForInsert(lcpLen + 1 /*indexLen*/,
                                                                 minKey /*key*/,
                ...
                if (parent_pos !=nullptr) {
                    uint32_t tmp_pos = allPositions1[ilen - 1];
                    if (m_hashTable.ht[tmp_pos].IsEqualNoHash(value, ilen)) {
                        *parent_pos = tmp_pos;
                    } else {
                        *parent_pos = allPositions2[ilen - 1];
                    }
                }
                ...
                if (minKeyUpdated) {
                    for (ilen--; ilen > 2; ilen--) {
                        uint32_t tmp_pos = allPositions1[ilen - 1];
                        if (m_hashTable.ht[tmp_pos].IsEqualNoHash(value, ilen)) {
                            assert(m_hashTable.ht[tmp_pos].GetIndexKeyLen() == ilen);
                            if (value < m_hashTable.ht[tmp_pos].minKey) {
                                m_hashTable.ht[tmp_pos].minKey = value;
                            } else {
                                break;
                            }
                        } else {
                            tmp_pos = allPositions2[ilen - 1];
                            if (m_hashTable.ht[tmp_pos].IsEqualNoHash(value, ilen)) {
                                assert(m_hashTable.ht[tmp_pos].GetIndexKeyLen() == ilen);
                                if (value < m_hashTable.ht[tmp_pos].minKey) {
                                    m_hashTable.ht[tmp_pos].minKey = value;
        ...
        //
        uint32_t inserted_leaf_node;
        {
            bool exist, failed;
            inserted_leaf_node = m_hashTable.Insert(lcpLen + 1 /*indexLen*/,
                                                    8 /*fullKeyLen*/,
                                                    value /*minKey*/,
                                                    -1 /*firstChild*/,
                                                    exist /*out*/,
            ...
            if (inserted_leaf_pos != nullptr) {
                *inserted_leaf_pos = inserted_leaf_node;
            }
        ...
        if (lcpLen == 2) {
            ...
            if (insert_write_set == nullptr) {
                COND_INC_VERSION(m_hashTable.ht[inserted_leaf_node].version_lock);
                UNLOCK_TOP_LEVELS();
            }
            return true;
        }
        ...
        if (insert_write_set == nullptr)
            UNLOCK_PATH_FROM_LCP()

        if (x_used) {
            if (insert_write_set == nullptr)
                COND_INC_VERSION(m_hashTable.ht[x].version_lock);
            //m_hashTable.ht[x].version_lock += m_hashTable.ht[x].version_lock % 2;
        }
        if (insert_write_set == nullptr)
            COND_INC_VERSION(m_hashTable.ht[inserted_leaf_node].version_lock);
        //m_hashTable.ht[inserted_leaf_node].version_lock += m_hashTable.ht[inserted_leaf_node].version_lock % 2;
        return true;
    }

    bool MlpSet::Exist(uint64_t value) {
#define VValidateTopLevels() if(top_levels_read_set[0] != SAFE_LOAD(this->m_root_version) ||\
        top_levels_read_set[1] != SAFE_LOAD(this->m_treeDepth1_version) ||\
        top_levels_read_set[2] != SAFE_LOAD(this->m_treeDepth2_version)) goto _retry;
        ...
        uint32_t *expectedHash = reinterpret_cast<uint32_t *>(_expectedHash);
        uint32_t top_levels_read_set[3];

        _retry:
        top_levels_read_set[0] = SAFE_LOAD(this->m_root_version);
        top_levels_read_set[1] = SAFE_LOAD(this->m_treeDepth1_version);
        top_levels_read_set[2] = SAFE_LOAD(this->m_treeDepth2_version);

        for (int i = 0; i < 3; i++) {
            if (top_levels_read_set[i] % 2)
                goto _retry;
        }
        ...
        int lcpLen = m_hashTable.QueryLCP(value,
                                          ...
                                          false);
        if (lcpLen == 2) {
            bool result = IS_VAL_COVERED_BY_TOP_LEVELS(value);
            VValidateTopLevels()
            return result;
        }
...
#define VValidateTopLevels() if(top_levels_read_set[0] != SAFE_LOAD(this->m_root_version) ||\
        ...
        int lcpLen = m_hashTable.QueryLCP(value,
                                          ...
                                          allPositions[1] /*out*/,
                                          expectedHash /*out*/,
                                          reinterpret_cast<uint32_t *>(&read_set),
                                          reinterpret_cast<CuckooHashTableNode **>(&ptr_db),
                                          false);
        if (lcpLen == 8) {
            ValidateVersion()
    ...
    void MlpSet::InsertRange(uint64_t low, uint64_t high) {
        ...
        assert((low % 256 == 0 && (high + 1) % 256 == 0) || (low == high));

        vector<uint64_t> low_values, high_values;
        vector<uint8_t> sb_values;
        this->m_globalLock.lock();
        if (low == high) {
            Insert(low);
            this->m_globalLock.unlock();
            return;
        }

        /*
         * Decompose (low,high) to series of perfect ranges
         * that we can store in the trie
         */
        bool is_root_range_exist = handle_range(low_values, high_values, sb_values, low, high);

        assert(low_values.size() == high_values.size() && high_values.size() == sb_values.size());

        /*
         * Iterate all perfect ranges and store them,
         * one by one
         */

        InsertRangeWS write_set;

        for (int i = 0; i < 8; i++) {
            write_set.Insert1WS[i] = nullptr;
            write_set.Insert1WS[i + 8] = nullptr;

            write_set.Insert2WS[i] = nullptr;
            write_set.Insert2WS[i + 8] = nullptr;
        }


        bool from_top_to_low_levels = false;
        bool already_done_top_to_low = false;

        InsertPerfectRange(low_values[0], high_values[0], sb_values[0], is_root_range_exist, &write_set);

        for (uint64_t i = 1; i < low_values.size(); i++) {
            if (sb_values[i - 1] <= 2 && sb_values[i] >= 2 && !already_done_top_to_low) {
                from_top_to_low_levels = true;
                already_done_top_to_low = true;
            }
            uint64_t curr_low = low_values[i];
            uint64_t curr_high = high_values[i];
            uint8_t curr_sb = sb_values[i];
            if (!from_top_to_low_levels) {
                InsertPerfectRange(curr_low, curr_high, curr_sb, false, nullptr);
            }
            else {
                InsertPerfectRange(curr_low, curr_high, curr_sb, is_root_range_exist, &write_set);
                from_top_to_low_levels = false;
            }

        }

        //Release write_set
        int SplitNode1_index_len = !write_set.Insert1SplitNodePOS_flag ? 0
                                                                       : m_hashTable.ht[write_set.Insert1SplitNodePOS].GetIndexKeyLen();
        int SplitNode2_index_len = !write_set.Insert2SplitNodePOS_flag ? 0
                                                                       : m_hashTable.ht[write_set.Insert2SplitNodePOS].GetIndexKeyLen();

        for (int i = 2; i < 8; i++) {
            if (SplitNode1_index_len == i)
                COND_INC_VERSION(m_hashTable.ht[write_set.Insert1SplitNodePOS].version_lock);

            if (SplitNode2_index_len == i)
                COND_INC_VERSION(m_hashTable.ht[write_set.Insert2SplitNodePOS].version_lock);

            if (write_set.Insert1WS[i] != nullptr) {
                COND_INC_VERSION(write_set.Insert1WS[i]->version_lock);
                COND_INC_VERSION(write_set.Insert1WS[i + 8]->version_lock);
            }

            if (write_set.Insert2WS[i] != nullptr) {
                COND_INC_VERSION(write_set.Insert2WS[i]->version_lock);
                COND_INC_VERSION(write_set.Insert2WS[i + 8]->version_lock);
            }

        }


        COND_INC_VERSION(this->m_root_version);
        COND_INC_VERSION(this->m_treeDepth1_version);
        COND_INC_VERSION(this->m_treeDepth2_version);
     ...
     */
    void MlpSet::InsertPerfectRange(uint64_t low, uint64_t high, uint8_t sb, bool handle_range_root,
                                    InsertRangeWS *write_set) {
        ...
        /*
         * NOT SURE WHY,
         * but i found a case where sb needs to be updated,
         * so:
         */
        sb = get_split_byte_index(low, high);
...
#define to_byte next_byte_after_sb_in_high // same..
        ...
        switch (sb) {
            case 0 ... 2:
                ...
                COND_LOCKED_VERSION(m_treeDepth1_version);
                COND_LOCKED_VERSION(m_treeDepth2_version);
                COND_LOCKED_VERSION(m_root_version);
                ...
                for (uint32_t B = td1_l; B <= td1_r; B++) {
                    ...
                    m_treeDepth1[B / 64] |= uint64_t(1) << (B % 64);
                    //m_treeDepth1[h16bits / 64] |= uint64_t(1) << (h16bits % 64);
                }

                for (uint32_t B = td2_l; B <= td2_r; B++) {
                    m_treeDepth2[B / 64] |= uint64_t(1) << (B % 64);
                }

                break;
            case 3 ... 7:
                uint32_t leaf_to_del_pos = 0;
                uint32_t parent_pos;
                bool split_flag = false;

                //TODO: Handle the case where pos1 and pos2 might not be accurate as displacment might invalidate on of them.

                if (handle_range_root) {
                    Insert(low, nullptr, &leaf_to_del_pos, (CuckooHashTableNode**) &write_set->Insert1WS);
                    Insert(high, &parent_pos, &leaf_to_del_pos,(CuckooHashTableNode**) &write_set->Insert2WS);
                } else {
                    Insert(low, nullptr, &leaf_to_del_pos);
                    Insert(high, &parent_pos, &leaf_to_del_pos);
                }

                bool temp;
                uint32_t high_pos = leaf_to_del_pos;
                uint8_t low_index_len = m_hashTable.ht[high_pos].GetIndexKeyLen();
                uint32_t low_pos = m_hashTable.Lookup(low_index_len, low, temp);

                m_hashTable.ht[low_pos].hash = 0;
                SAFE_STORE(m_hashTable.ht[low_pos].version_lock, 0);

                m_hashTable.ht[high_pos].hash = 0;
                SAFE_STORE(m_hashTable.ht[high_pos].version_lock, 0);
                 ...
                 */
                bool found;
                if (this->m_hashTable.ht[parent_pos].IsUsingInternalChildMap())
                    this->m_hashTable.ht[parent_pos].ExtendToBitMap();

                /*
                 * Now we have a node with empty bitmap that waits to be updated
                 */
                for (uint16_t B = from_byte; B <= to_byte; B++)
                    this->m_hashTable.ht[parent_pos].BitMapSet(B);

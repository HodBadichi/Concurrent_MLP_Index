...
#pragma pack(push, 1) // Disable structure padding
    ...
    struct CuckooHashTableNode {
        ...
        //Version number :
        // Odd value represents an inconsistent state of the slot.
        // Even value represents a consistent state of the slot.
        std::atomic<uint8_t> version_lock;
        ...
        bool IsEqualNoHash(uint64_t key, int len) {
            return ((hash & 0xf8000000U) == (0x80000000U | uint32_t(((len) - 1) << 27)) &&
                    (key >> (64 - 8 * len)) == (minKey >> (64 - 8 * len)));
        }
	...
	void PrintSelf();
	void GetChildMapStr(char *s);
    };

#pragma pack(pop) // Restore default structure alignment

    struct InsertRangeWS {
        CuckooHashTableNode *Insert1WS[16];
        uint32_t Insert1LeafPOS;
        uint32_t Insert1SplitNodePOS;
        bool Insert1SplitNodePOS_flag = false;

        CuckooHashTableNode *Insert2WS[16];
        uint32_t Insert2LeafPOS;
        uint32_t Insert2SplitNodePOS;
        bool Insert2SplitNodePOS_flag = false;
    };

    static_assert(sizeof(CuckooHashTableNode) - 1 == 24, "size of node should be 24");
...
//
    ...
    public:
        ...
        public:
            LookupMustExistPromise() : valid(0), fake(0) {}

            LookupMustExistPromise(CuckooHashTableNode *h)
                    : valid(1), fake(0), h1(h), h2(nullptr) {}

            /*
             * Constructor for fake promise
             */
            LookupMustExistPromise(uint64_t key) : valid(1), fake(1), shiftedKey(key) {}
            ...
            LookupMustExistPromise(uint16_t valid, uint16_t shiftLen,
                    ...
                    : valid(valid), fake(0), shiftLen(shiftLen), h1(h1), h2(h2), expectedHash(expectedHash),
            ...
            uint64_t Resolve() {
                ...
                if (IsFake())
                    return shiftedKey;
                else if (h2 == nullptr || h1->IsEqual(expectedHash, shiftLen, shiftedKey)) {
                    return h1->minKey;
                } else {
                    assert(h2->IsEqual(expectedHash, shiftLen, shiftedKey));
                    return h2->minKey;
                }
            }
        ...
        int QueryLCP(uint64_t key,
                     ...
                     bool ignore_range_mechanic);

        int NaiveQueryLCP(uint64_t key,
                          uint32_t &idxLen,
                          uint32_t *allPositions1,
                          uint32_t *allPositions2,
                          uint32_t *expectedHash,
    ...
    public:
        ...
        void PrintMlpNode(CuckooHashTableNode*);
        void PrintMlpSet();
	uint64_t CountNodes();

        void GetChildMapStr(char *s, CuckooHashTableNode *Node);
        ...
        // Insert a range. Always succeeds since we assume that input contains no overlapping ranges TODO: Examine this statement.
        void InsertRange(uint64_t low, uint64_t high);

        // This function assumes that low[sb*8+1:] is all 0x00 and high[sb*8+1:] is all 0xff.
        //@sb = split byte. i.e. the byte index that after it low and high diverges.
        void
        InsertPerfectRange(uint64_t low, uint64_t high, uint8_t sb, bool handle_range_root, InsertRangeWS *write_set);
    ...
    private:
        ...
        bool Insert(uint64_t value, uint32_t *parent = nullptr, uint32_t *inserted_leaf_pos = nullptr, CuckooHashTableNode **insert_write_set = nullptr);

...
int _handle_range(uint64_t* x, uint64_t* y, uint64_t *lf1, uint64_t *lf2, uint8_t *sb)
{
    assert(x <= y);
...
bool handle_range(vector<uint64_t> &X, vector<uint64_t> &Y, vector<uint8_t> &SB, uint64_t low, uint64_t high)
{
    uint8_t sb;
    uint64_t lf1[2], lf2[2];
    vector<vector<uint64_t>> Job({vector<uint64_t>({low, high})});
    bool ret_val = true;

    while(!Job.empty()) {
        vector<uint64_t> curr_job = Job.back();
        Job.pop_back();

        int range_status = _handle_range(&curr_job[0], &curr_job[1], lf1, lf2, &sb);

        /*
         * Add next perfect range (and sb) to output,
	 * but only if the current perfect range is valid
         */
	if (curr_job[0] < curr_job[1]) {
		X.push_back(curr_job[0]);
		Y.push_back(curr_job[1]);
		SB.push_back(sb);
	} else ret_val = false;

         if (LEFTOVER1_UPDATED == range_status || BOTH_LEFTOVERS_UPDATED == range_status) {
            if ((lf1[1] - lf1[0] + 1) >= 256)
                Job.push_back(vector<uint64_t>({lf1[0], lf1[1]}));
            else
                assert(false);
        }
        if (LEFTOVER2_UPDATED == range_status || BOTH_LEFTOVERS_UPDATED == range_status) {
            if ((lf2[1] - lf2[0] + 1) >= 256)
                Job.push_back(vector<uint64_t>({lf2[0], lf2[1]}));
            else
                assert(false);
        }
    }

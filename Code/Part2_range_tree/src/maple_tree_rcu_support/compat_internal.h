...
static inline void *kmem_cache_alloc(struct kmem_cache * a, gfp_t b) {
	void *ptr;
	if (posix_memalign(&ptr, sizeof(struct maple_node), sizeof(struct maple_node)) != 0)
		return NULL;
	return ptr;
}
static inline void kmem_cache_free(struct kmem_cache * a, void *ptr) {
	free(ptr);
}
...
static inline int kmem_cache_alloc_bulk(struct kmem_cache *s, gfp_t b, size_t size, void **ptr) {
	size_t i;
	for (i = 0; i < size; i++)

...
struct maple_tree {
	union {
        volatile pthread_spinlock_t*	ma_lock;
...
/*
 * Global writer lock reference
 */
extern volatile pthread_spinlock_t global_writer_lock;
...
#define DEFINE_RCU_MTREE(name)						\
	struct maple_tree name = MTREE_INIT(name, MT_FLAGS_USE_RCU); \
    pthread_spin_init(name.ma_lock, PTHREAD_PROCESS_SHARED);

#define CLEAR_RCU_MTREE(name) \
    pthread_spin_destroy(name.ma_lock);
